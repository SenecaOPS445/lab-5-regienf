...
def write_file_list(file_name, list_of_lines):
    ...
    f = open(file_name, 'w')
    for line in list_of_lines:  # for each line list
        f.write(line + '\n')
    f.close()
...
def copy_file_add_line_numbers(file_name_read, file_name_write):
    ...
    lines = fr.readlines()
    line_numb = 1           # initialize line counter
    for line in lines:
        line = line.strip()
        fw.write(f"{line_numb}:{line}\n")
        line_numb += 1      # increment

    fr.close()              # close source
    fw.close()              # close destination
